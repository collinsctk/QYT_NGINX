...
# 将根证书、服务器证书和私钥打包成PKCS#12格式，使用兼容的PBE算法和SHA1哈希算法
system_cmd(f'openssl pkcs12 -export -out {input_domain.strip()}.p12 -inkey server-key.pem -in server.pem -certfile /opt/certs/ca.cer -passout pass:Cisc0123 -keypbe PBE-SHA1-3DES -certpbe PBE-SHA1-3DES -macalg sha1')
...
print(f'明文私钥文件: {os.getcwd()}/server-key.pem')
